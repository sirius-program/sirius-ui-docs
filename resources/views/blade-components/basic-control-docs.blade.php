<x-layouts::app :title="$title">
    @php
        ...
        $scenarios = [
            'input' => 'Set up a project and reserve seats for your team.',
            'password' => 'Create a password for a new workspace account.',
            'textarea' => 'Write a brief for the next project milestone.',
            'checkbox' => 'Invite a teammate, assign permissions, and review project notification selection.',
            'radio' => 'Choose a subscription plan for your workspace.',
            'switch' => 'Enable the security alerts required for a workspace account.',
        ];
    @endphp
        ...
        <article class="mx-auto flex min-w-0 max-w-4xl flex-col gap-8">
            ...
            @foreach ($examples as $example => $label)
                <section id="{{ $example }}" data-control-demo="{{ $example }}" class="space-y-5">
                    ...
                    <p class="text-zinc-600 dark:text-zinc-400">{{ $scenarios[$example] }}</p>
