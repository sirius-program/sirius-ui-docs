...
it('renders each basic control documentation page', function (string $control, array $examples): void {
    $this->get(route('components.control', ['control' => $control]))
        ->assertSee(array_map(fn (string $example): string => 'Interactive ' . $example . ' example', $examples))
...
it('redirects former component pages to their combined documentation', function (string $old, string $destination): void {
    $this->get(route('components.control', ['control' => $old]))
        ->assertRedirect(route('components.control', ['control' => $destination]));
...
it('validates ordinary submission and normalizes omitted boolean fields', function (): void {
    $this->post(route('components.basic.store'), ['title' => 'Example', 'quantity' => '0', 'plan' => '0', 'roles' => ['0', 'editor']])
        ->assertRedirect(route('components.control', ['control' => 'input']))
...
it('rejects invalid ordinary form values in the named error bag', function (): void {
    $this->post(route('components.basic.store'), ['title' => '', 'quantity' => -1, 'plan' => 'invalid', 'roles' => ['admin']])
